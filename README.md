...
Environment Setup
Copy .env.example to .env.local for Next.js and .env for API/server usage.
Fill database and auth keys.
Optional: fill AWS and/or Cloudinary credentials to enable real signed uploads.
Database Setup
npm run db:migrate
This applies db/migrations/001_init.sql and creates:
